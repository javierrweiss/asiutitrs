 ...
 * @type {Number}
 *
 * @properties={typeid:35,uuid:"9225775D-CBE7-437D-B9DE-6F72705C9583",variableType:4}
 */
var tipo_nomencla = 0;
 ...
 * @type {Number}
 *
 * @properties={typeid:35,uuid:"BCD90693-B22C-4BE7-9B9D-48031DBB6E82",variableType:4}
 */
 ...
 * @type {Date}
 *
 * @properties={typeid:35,uuid:"FB864EEF-7C99-45B1-8DE2-41B3E4DB8A6D",variableType:93}
 */
var fecha_impresion = new Date();

/**
 * @properties={typeid:24,uuid:"D9C34764-23D7-479A-B605-900AA139CE62"}
 * @AllowToRunInFind
 */
function sub_buildReport() {
	...
	if(reimpresor_vhiscli_to_tbc_hcicir_new.getSize()>0){
		...
		fs3.find()
		fs3['medpercod']=reimpresor_vhiscli_to_tbc_hcicir_new.hcirlegajomed
		fs3.search(true)
		...
		html += '<td width=25%>' + reimpresor_vhiscli_to_tbc_hcicir_new.hcirhematocrito +'</td>'
		html += '<td width=25%>' + 'Recuento de Blancos: '+'</td>'
		html += '<td width=25%>' + reimpresor_vhiscli_to_tbc_hcicir_new.hcirgloblancos +'</td>'
		html += '</tr>'
		html += '</table>'	
		globals.lineas_impre++
		globals.tempHTML = html
		...
		html += '<td width=25%>' + reimpresor_vhiscli_to_tbc_hcicir_new.hcirglucemia +'</td>'
		html += '<td width=25%>' + 'Urea: '+'</td>'
		html += '<td width=25%>' + reimpresor_vhiscli_to_tbc_hcicir_new.hcirurea +'</td>'
		html += '</tr>'
		html += '</table>'	
		globals.lineas_impre++
		globals.tempHTML = html
		...
		html += '<td width=25%>' + reimpresor_vhiscli_to_tbc_hcicir_new.hcircoagusangria +'</td>'
		html += '<td width=25%>' + 'Protrombina: '+'</td>'
		html += '<td width=25%>' + reimpresor_vhiscli_to_tbc_hcicir_new.hcirprotrombina +'</td>'
		html += '</tr>'
		html += '</table>'	
		globals.lineas_impre++
		globals.tempHTML = html
		...
		if(reimpresor_vhiscli_to_tbc_hcicir_new.hcirmonitoreo.charAt(0)=='S'||reimpresor_vhiscli_to_tbc_hcicir_new.hcirmonitoreo.charAt(0)=='1'){
			monitoreo_sino='Si'
		}else{
			monitoreo_sino='No'
		}
		if(reimpresor_vhiscli_to_tbc_hcicir_new.hcirconsen.charAt(0)=='S'||reimpresor_vhiscli_to_tbc_hcicir_new.hcirconsen.charAt(0)=='1'){
		...
		if(reimpresor_vhiscli_to_tbc_hcicir_new.hcirantite.charAt(0)=='S'||reimpresor_vhiscli_to_tbc_hcicir_new.hcirantite.charAt(0)=='1'){
			antitetanica_sino='Si'
		}else{
			antitetanica_sino='No'
		}
		if(reimpresor_vhiscli_to_tbc_hcicir_new.hcirdador.charAt(0)=='S'||reimpresor_vhiscli_to_tbc_hcicir_new.hcirdador.charAt(0)=='1'){
			dadores_sino='Si'
		}else{
			dadores_sino='No'
		}
...
/**
 * @properties={typeid:24,uuid:"8192ED32-3BCB-495C-971A-46D873E1A3BD"}
 */
...
/**
 * @properties={typeid:24,uuid:"66C1E3D3-766B-4CB1-8B69-42D901A6BE45"}
 */
...
/**
 * @properties={typeid:24,uuid:"268F2329-4202-41AB-92C0-863C5BEAA91A"}
 */
function lleno_dataset_texto_hcicir() {
	var query = "select hcirx_linea from tbc_hcicirtxt where hcirx_histclin = ? and hcirx_tiphistclin = ? and hcirx_fec = ? and hcirx_hora = ? and hcirx_tittxt = ?";
	var args = new Array();
	args[0]=globals.vHiscli;
	args[1]=1;
	args[2]=globals.reimpresor_fecha_ingreso;
	args[3]=globals.reimpresor_hora_ingreso;
	args[4]=globals.grupo_texto
    
	var ds = databaseManager.getDataSetByQuery("asistencial", query, args, 100);
	...
	if(ds.getMaxRowIndex()>0){
		for (var i = 1 ; i <=ds.getMaxRowIndex() ; i++ )
		{
			html += '<table width=100% border=0 cellpadding=1 cellspacing=0>'
			html += '<tr>'
			html += '<td width=100%>' + ds.getValue(i,1) + '</td>'
			html += '</tr>'
			html += '</table>'	
			globals.lineas_impre++
			globals.tempHTML = html
