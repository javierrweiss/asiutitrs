 ...
 * @type {String}
 *
 * @properties={typeid:35,uuid:"20A24CFC-E825-4D52-B81B-21532648EE1A"}
 */
var f_presenta = ' ';
 ...
 * @type {String}
 *
 * @properties={typeid:35,uuid:"FD649DED-5922-47C4-8E42-B83B46847D67"}
 */
var f_descrip = ' ';
 ...
 * @type {String}
 *
 * @properties={typeid:35,uuid:"411107D7-664C-454E-B613-AF7D22BEEBDF"}
 */
var fNroInternacion = ' ';
 ...
 * @type {String}
 *
 * @properties={typeid:35,uuid:"BBD23E25-96EB-4F4F-AD49-459A2DC8C3D1"}
 */
var fTipoPaciente = ' ';
 ...
 * @type {String}
 *
 * @properties={typeid:35,uuid:"1021BF8A-46E4-4633-AF59-2B427A06DB1B"}
 */
 ...
 * @type {String}
 *
 * @properties={typeid:35,uuid:"0708D2DD-5372-4B5E-9F8F-38F821236844"}
 */
var fSexo = '';
 ...
 * @type {String}
 *
 * @properties={typeid:35,uuid:"2974F724-5EA2-4F32-A7F9-15962193443A"}
 */
var fEdad = '';
 ...
 * @type {String}
 *
 * @properties={typeid:35,uuid:"DCF2F812-34DD-4D75-BFE1-431BFCA342B3"}
 */
var fObraSocial = '';
 ...
 * @type {String}
 *
 * @properties={typeid:35,uuid:"82E5DCCB-4FB0-4F45-B842-4E078B588BAE"}
 */
 ...
 * @type {Number}
 *
 * @properties={typeid:35,uuid:"19E9137B-6132-46C1-8FBB-E7A68B633D40",variableType:4}
 */
var vNroAnestesia = 0;
 ...
 * @type {String}
 *
 * @properties={typeid:35,uuid:"3633608A-ED12-4320-970D-C1DEE406B5D7"}
 */
var vString = '';
 ...
 * @type {String}
 *
 * @properties={typeid:35,uuid:"754C163F-B1CB-42F8-B9FA-078C471B3012"}
 */
var vDatosQueFaltan = '';

/**
 * @properties={typeid:35,uuid:"D04541BF-93DC-47EB-B3F2-884E6AC7A22C",variableType:-4}
 */
 ...
 *
 * @properties={typeid:24,uuid:"A00D3EBD-5D05-423F-9F2F-CF24B44C5490"}
 * @AllowToRunInFind
 */
		...
		var thePressedButton = plugins.dialogs.showQuestionDialog('Confirma Grabación del '+ tipoarticulo, '¿Confirma la grabación del '+ globals.bandejaciru_vCodigoMed + ' - ' + bandejaciru_vcodigomed_to_tbc_articulos.art_descrip +' ?','Si','No');
	 	if (thePressedButton == "Si"){ 
	 		if(bandejaciru_vestado2259_to_tbc_his_lectora.getSize()<1){
				...
				bandejaciru_vestado2259_to_tbc_his_lectora.hlec_estado=globals.bandejaciru_estado
			...
			if(globals.bandejaciru_TipHisclin==0){
				vProtocoloOk=true
				if(bandejaciru_vs4_to_tbc_cirugint.getSize()<1){
					$cierreOk=false
				}
				...
				if(bandejaciru_vs4_to_tbc_cirugint.cirianestesia==1){
 ...
 * @param {JSEvent} event the event that triggered the action
 *
 * @properties={typeid:24,uuid:"9921CC50-D5E8-45F2-A14C-DCBCB1F80841"}
 * @AllowToRunInFind
 */
 ...
 * @param {JSEvent} event the event that triggered the action
 *
 * @properties={typeid:24,uuid:"1A841F1F-FFF5-4432-8FBE-DD40EDD4517B"}
 * @AllowToRunInFind
 */
function onShow_IngresoMedicamentos_tab(firstShow, event) {
	...
	if((bandejaciru_vhiscli_to_tbc_ciruguar.getSize()>0&&globals.bandejaciru_TipHisclin==1)||(bandejaciru_vs4_to_tbc_cirugint.getSize()>0&&globals.bandejaciru_TipHisclin==0)){
		elements.tabless.tabIndex=1
		...
		if((bandejaciru_vhiscli_to_tbc_ciruguar.cirgusoanestesia==1&&globals.bandejaciru_TipHisclin==1)||(bandejaciru_vs4_to_tbc_cirugint.cirianestesia==1&&globals.bandejaciru_TipHisclin==0)){
			elements.combo_Imed.enabled=true
			plugins.dialogs.showInfoDialog("Atención","Debe seleccionar el tipo de Medicamento Cirugía ó Anestesia")
			elements.combo_Imed.requestFocus()
		}else{
 ...
 * @param {JSEvent} event the event that triggered the action
 *
 * @properties={typeid:24,uuid:"F9DAB2C0-AA95-4532-9FB8-4E5162DB0C96"}
 * @AllowToRunInFind
 */
 ...
 * @param {JSEvent} event the event that triggered the action
 *
 * @properties={typeid:24,uuid:"5B6BD55C-00B6-40FA-910D-25483D853CC1"}
 */
function onAction_volver(event) {
	...
	var $win=application.getWindow(activeWindow.getName())
	if($win!=null){
		$win.hide()
		$win.destroy()
	}
 ...
 * @param {JSEvent} event the event that triggered the action
 *
 * @properties={typeid:24,uuid:"9AF30721-EFCA-4444-9CC0-AAF472A3C9E0"}
 * @AllowToRunInFind
 */
function onAction_btn_cierre(event) {
	...
	if(globals.bandejaciru_TipHisclin==0){
		if(bandejaciru_vs4_to_tbc_cirugint.getSize()<1){
			itemsPendientes+='No dió de Alta al Protocolo '+'\n'
		}
		ww = ww + 10
		elements.bean_243.value = ww
		application.updateUI()
	...
	if(itemsPendientes!=''){
		itemsPendientes+='No se puede Cerrar el Protocolo.'
	...
	if(globals.bandejaciru_TipHisclin==1){
		try{
			...
			application.output(msg.message)
			plugins.dialogs.showErrorDialog("grabo_Reposicion",msg.message)
		}
		ww = ww + 10
		elements.bean_243.value = ww
		application.updateUI()
	...
	if(globals.bandejaciru_TipHisclin==0){
		try{
			Actualiza_His_Lectora()
		}catch(msg){
			application.output(msg.message)
			plugins.dialogs.showErrorDialog("Actualiza_His_Lectora()",msg.message)
		}
		ww = ww + 20
		elements.bean_243.value = ww
		application.updateUI()
		try{
			...
			application.output(msg.message)
			plugins.dialogs.showErrorDialog("EjecutaRunCobol()",msg.message)
		}
		ww = ww + 20
		elements.bean_243.value = ww
		application.updateUI()
 ...
 * @returns {Boolean}
 *
 * @properties={typeid:24,uuid:"F4632FBD-DB97-4F33-8220-72541DD16E58"}
 */
 ...
 * @param {JSEvent} event the event that triggered the action
 *
 * @properties={typeid:24,uuid:"CAEE2BE7-A0C0-4C95-8722-21E923C26857"}
 */
...
/**
 * @properties={typeid:24,uuid:"E743A179-4968-41E8-9A7A-AB2131185349"}
 */
 ...
 * @param {Number} tipo
 *
 * @properties={typeid:24,uuid:"91A24149-4B06-46B8-AA07-FA48DBED13DF"}
 */
...
/**
 * @properties={typeid:24,uuid:"C5B04FF9-18E0-46B9-BAA2-0CD638185B93"}
 */
 ...
 * @param {Number} grupo
 *
 * @properties={typeid:24,uuid:"22C731A9-8307-46EA-BD37-688502EC193F"}
 */
...
/**
 * @properties={typeid:24,uuid:"C70F63B4-691D-4A4B-8C1F-83DB43B73CEF"}
 */
 ...
 * @AllowToRunInFind
 *
 * @properties={typeid:24,uuid:"D26C9CB8-B4CB-4A6B-9FFC-47A1DE8DB9A8"}
 */
...
/**
 * @properties={typeid:24,uuid:"24B15BB6-7527-4C33-8A18-39D7EC733584"}
 */
...
/**
 * @properties={typeid:24,uuid:"4A6E6F1B-45B4-4C2E-97F5-918E1BB8383B"}
 */
...
/**
 * @properties={typeid:24,uuid:"B4E9EAE3-ED43-4D76-A452-CCFEC7194275"}
 */
...
/**
 * @properties={typeid:24,uuid:"E66A7D0D-1DA4-47C9-98A1-A394FBC4DB6D"}
 */
...
/**
 * @properties={typeid:24,uuid:"964F6062-ED2A-4F0B-B512-96165AE2F4BC"}
 * @AllowToRunInFind
 */
function grabo_Reposicion(){
	//var $numero_rep_ant=0;
	globals.bandejaciru_numero_rep=0;
	Actualiza_numerador_reposicion()
	...
	//var fs_repo =databaseManager.getFoundSet("asistencial","tbc_reposicion")
	var largo_his_lectora = 0
	//var largo_repos = 0
	var largo_numero = 0
	for(var j=1;j<=2;j++){
		...
		fs['hlec_fecing']=globals.bandejaciru_fechaingreso
		fs['hlec_horing']=globals.bandejaciru_horaingreso
		fs['hlec_imed']=globals.bandejaciru_vImed
		fs['hlec_sector']=globals.bandejaciru_vSector
		fs['hlec_estado']=globals.bandejaciru_vEstado
		fs['hlec_cant']= '>0'
		fs.search()
		...
		if(largo_his_lectora> 0){
			...
			for(var i = 1; i <= largo_his_lectora;i++){
				...
				if(bandejaciru_univoco_to_tbc_reposicion.getSize()<1){
					...
					globals.bandejaciru_artsol_rep=fs['hlec_art']
					bandejaciru_univoco_to_tbc_reposicion.repartsol=fs['hlec_art']
					bandejaciru_univoco_to_tbc_reposicion.repartsol3=fs['hlec_art']
					...
					bandejaciru_univoco_to_tbc_reposicion.repsecsol=fs['hlec_sector']
					bandejaciru_univoco_to_tbc_reposicion.repsecsol3=fs['hlec_sector']
					bandejaciru_univoco_to_tbc_reposicion.repsecpro=fs['hlec_sector']
					bandejaciru_univoco_to_tbc_reposicion.replegsol=globals.vLega
					bandejaciru_univoco_to_tbc_reposicion.repcansol=fs['hlec_cant']
					bandejaciru_univoco_to_tbc_reposicion.reptiphis=1
					bandejaciru_univoco_to_tbc_reposicion.rephiscli=globals.vHiscli
					
					try{
						databaseManager.saveData(bandejaciru_univoco_to_tbc_reposicion.getRecord(1))
					}catch(msg){
						application.output(msg.message)
					}
				...
				fs['hlec_protocolo']=globals.bandejaciru_protocolo
				fs['hlec_nrorep']=globals.bandejaciru_numero_rep
				try{
					databaseManager.saveData(fs)
				}catch(msg){
					application.output(msg.message)
				...
				for (jj = 0; jj < array1.length; jj++) {
					...
					globals.DIALOGS.showErrorDialog("Error de Grabación",'Tabla:' + tableSQLName1 + ' en server:' + jstable1.getServerName() + ' falló la grabación. Avise urgente a Sistemas, por favor!')
					globals.DIALOGS.showErrorDialog("Error en grabación de tbc_his_lectora",record1.exception.getMessage());
					//application.output(record1.exception);
					if (record1.exception.getErrorCode() == ServoyException.RECORD_VALIDATION_FAILED) {
					// exception thrown in pre-insert/update/delete event method
 ...
 * @returns {Boolean}
 *
 * @properties={typeid:24,uuid:"026B8573-D004-481A-82BD-F15AA6AD3F52"}
 */
...
/**
 * @properties={typeid:24,uuid:"A05F8ECC-BA5D-4ACD-B9DB-81433841BB8F"}
 */
...
/**
 * @properties={typeid:24,uuid:"B05E2AC7-9C79-43AB-AA19-1C475FCB2338"}
 */
 ...
 * @param {JSEvent} event the event that triggered the action
 *
 * @properties={typeid:24,uuid:"E1CBB246-BF0D-45BC-BFE5-A11B673F4ED9"}
 */
...
/**
 * @properties={typeid:24,uuid:"43C07069-2465-4962-904D-72F5E1571B19"}
 */
function Controla_Codigos_Cirugia() {
    ...
    var args = new Array();
    args[0]=globals.vHiscli;
    args[1]=globals.bandejaciru_protocolo;
    var dataset = new Array();
 ...
 * @AllowToRunInFind
 *
 * @properties={typeid:24,uuid:"8F20236E-14EC-41EE-BDCC-67B9D6A9416D"}
 */
...
/**
 * @properties={typeid:24,uuid:"856AA854-6764-48C3-AB55-4C93FA9CD45A"}
 */
...
/**
 * @properties={typeid:24,uuid:"58FBEF81-9EB2-4351-95DC-035240220395"}
 */
...
/**
 * @properties={typeid:24,uuid:"710C20C3-5BE6-4DDF-A4A2-0149C77B6D0F"}
 */
 ...
 * @AllowToRunInFind
 *
 * @properties={typeid:24,uuid:"F468D856-D68C-4732-AD19-74E06A41A08F"}
 */
function EjecutaRunCobol() {
	var fs = databaseManager.getFoundSet("asistencial","tbc_cirugint")
	fs.find()
	fs['cirihistcl']=globals.vHiscli
	fs['cirifechacarga']=utils.stringToNumber(globals.bandejaciru_wkeycir.substr(10,8))
	fs['cirihoracarga']=utils.stringToNumber(globals.bandejaciru_wkeycir.substr(18,6))
	fs['ciritiponome']=0
	fs['ciricodnome']=0
	fs.search(true)
	...
	if(fs.getSize()>0){
		...
		plugins.dialogs.showInfoDialog("globals.bandejaciru_wkeycir",globals.bandejaciru_wkeycir)
	...
	vString = vServer.executeCommand("/app/servoy/scripts/shcob4000 "+servidor+" "+"1"+" "+globals.bandejaciru_wkeycir+" "+protocolo+" "+tipo+" "+legajo+" "+tipo_legajo);
...
/**
 * @properties={typeid:24,uuid:"D773D51D-2B8F-4ED4-B6A4-7D7173476BC9"}
 * @AllowToRunInFind
 */
function Actualiza_His_Lectora() {
	var $newEstado=0
	var $oldEstado=0
	var i = 0
	var jj = 0
	...
	var fs = databaseManager.getFoundSet("asistencial","tbc_his_lectora")
	fs.find()
	fs['hlec_hiscli']=globals.vHiscli
	fs['hlec_protocolo']=globals.bandejaciru_protocolo
	fs['hlec_sector']=globals.bandejaciru_vSector
	fs.search()
	
	var $largo_his_lectora = fs.getSize()
	
	if($largo_his_lectora>0){
		for(i=1;i<=$largo_his_lectora;i++){
			fs.setSelectedIndex(i)
			fs['hlec_estado']=$newEstado
		}
		try{
			databaseManager.saveData(fs)
		}catch(msg){
			plugins.dialogs.showErrorDialog("error al grabar his-lectora",msg.message)
		}
		var array1 = databaseManager.getFailedRecords(fs)
		for (jj = 0; jj < array1.length; jj++) {
			var record1 = array1[jj];
			...
			globals.DIALOGS.showErrorDialog("Error de Grabación",'Tabla:' + tableSQLName1 + ' en server:' + jstable1.getServerName() + ' falló la grabación. Avise urgente a Sistemas, por favor!')
			globals.DIALOGS.showErrorDialog("Error en grabación de tbc_his_lectora",record1.exception.getMessage());
			globals.DIALOGS.showErrorDialog("Error en grabación de tbc_his_lectora",bandejaciru_vestado2259_to_tbc_his_lectora.hlec_art.toString());
			//application.output(record1.exception);
			if (record1.exception.getErrorCode() == ServoyException.RECORD_VALIDATION_FAILED) {
			// exception thrown in pre-insert/update/delete event method
				var thrown1 = record1.exception.getErrorCode()
	...
	if($largo_his_lectora>0){
		...
		var array2 = databaseManager.getFailedRecords(fs)
		for (jj = 0; jj < array2.length; jj++) {
			var record2 = array2[jj];
			var jstable2 = databaseManager.getTable(record2);
			var tableSQLName2 = jstable2.getSQLName();
			globals.DIALOGS.showErrorDialog("Error de Grabación",'Tabla:' + tableSQLName2 + ' en server:' + jstable2.getServerName() + ' falló la grabación. Avise urgente a Sistemas, por favor!')
			globals.DIALOGS.showErrorDialog("Error en grabación de tbc_his_lectora",record2.exception.getMessage());
...
/**
 * @properties={typeid:24,uuid:"239A8558-5E42-4284-9FD6-E60E8E299F03"}
 */
function Actualiza_numerador_reposicion() {
